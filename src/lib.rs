...
use protobuf::{descriptor::FileDescriptorProto, plugin::code_generator_response::File, Message};
...
pub fn generate_py_init_configs(
    ...
    opts.as_ref()
        ...
        .for_each(|(opts, file_path)| {
            ...
            let py_module_path_parts: Vec<String> = file_descriptor
                .name()
                .split("/")
                .map(|part| part.replace(".proto", "_pb2"))
                .collect();

            let py_module_path: String = opts.parent_package.split(".")
                .map(String::from)
                .chain(py_module_path_parts.iter().cloned()).fold(String::new(), |acc: String, part: String| {
                    if acc.is_empty() {
                        part
                    } else {
                        format!("{}.{}", acc, part)
            ...
            log::info!("Python module path: {}", py_module_path);

            let python_imports = file_descriptor
                .message_type
                .iter()
                .filter(|_| opts.enable_top_level_imports && parent_dir.components().count() == 1)
                .map(|message| format!("from {} import {}", py_module_path, message.name()))
                .collect::<Vec<_>>();
...
pub fn generate_py_init_files(
    ...
    configs.into_iter().flat_map(|(path, config)| {
        // Read file if it already exists, otherwise create a new one.
...
mod tests {
    ...
    fn it_should_generate_py_init_configs_with_no_opts() {
        ...
        let result = generate_py_init_configs(&FileDescriptorProto::new(), &opts);
